...
const CategoryList = ({categoryName, operations, isExpanded, toggleCategory}) => (
    <div className="w-full">
        <h2 className="text-lg font-semibold p-4">{categoryName}</h2>
        <ul className="list">
            {(isExpanded ? operations : operations.slice(0, 5)).map((operation) => (
                <li key={operation.id} className="list-row p-2">
                    {operation.libelle}
                </li>
            ))}
            {operations.length > 5 && (
